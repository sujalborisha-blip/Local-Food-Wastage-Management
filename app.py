...
# --- PAGE 2: RUN 15 SQL QUERIES ---
    ...
    st.write("Select a query below to fetch live analytics from your MySQL database.")
        ...
        if conn:
            ...
            if not df_del.empty:
                ...
                if st.button("Delete Item ❌"):
                    ...
                    cursor = conn.cursor()
                    try:
                        cursor.execute("DELETE FROM claims WHERE Food_ID = %s", (del_id,))
...
# --- PAGE 4: SEARCH & FILTER ---
    ...
    conn = get_db_connection()
    if conn:
        # Fetch options from database for filter dropdowns
        ...
        # Button to search filtered data
